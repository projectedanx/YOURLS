...
/**
 * Outputs a menu item for the admin menu.
 *
 * @since 1.10.3
 * @param string $id    The menu item ID.
 * @param array  $data  The menu item data (url, anchor, title).
 * @param string $class The CSS classes for the li element.
 * @return void
 */
function yourls_html_menu_item( $id, $data, $class ) {
    if ( !isset( $data['url'] ) ) {
        return;
    }
    ...
    foreach( (array)$admin_links as $link => $ar ) {
        ...
        if( isset( $admin_sublinks[$link] ) ) {
            ...
            foreach( $admin_sublinks[$link] as $sublink => $subar ) {
                yourls_html_menu_item( $sublink, $subar, "admin_menu_sublevel admin_menu_sublevel_$sublink" );
            }
            echo "</ul>\n";
        }
    }
